...
struct phy_device {
	...
	int (*cfg_link)(struct phy_device *phy, bool autoneg,
	...
	int (*get_rx_latency)(struct phy_device *phy, int32_t link_speed,
			      int32_t *latency);
	int (*eee_enable)(struct phy_device *phy, bool enable,
...
int phy_modify_c22(struct phy_device *phy, uint8_t regnum, uint16_t mask,
		   uint16_t val);
