 ...
 *
 * This routine configures the QEP device for various parameters such as
 ...
 * @param config Pointer to QEP configuration structure.
 *
 ...
 *          configuration.
 * @retval -EBUSY if a decode/capture  operation on the device is
 *          in progress.
 ...
 * This function should not be called in interrupt context.
 * The counter value counts down from value =(4* pulses per revolution)
 ...
 * @retval -EPERM if the operation is not permitted in current
 *          configuration.
 * @retval -EBUSY if a decode/capture  operation on the device is in
 *          progress.
 ...
 *
 * This function queries the QEP controller for to determine if a phase
 * errors exists between inputs Phase A and Phase B.
 * This function should not be called in interrupt context.
 *
 *
 * @param dev QEP device structure.
 * @param p_phase_err pointer to uint32_t that would hold the phase error
 * status.
 *
 * @retval -EPERM if the operation is not permitted in current
 * configuration.
